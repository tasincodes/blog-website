...
def home(request):
    ...
    regular_blogs = Blog.objects.filter(category__is_premium=False).order_by('-created_date')
    premium_blogs = Blog.objects.filter(category__is_premium=True).order_by('-created_date')
    context = {
        "blogs": blogs,
        "tags": tags,
        "regular_blogs": regular_blogs,
        "premium_blogs": premium_blogs,
    }
...
def purchase_premium(request, slug):
    blog = Blog.objects.get(slug=slug)
    # You can set the price dynamically or have a fixed premium price
    premium_price = 999  # Example price in cents
    ...
    if request.method == 'POST':
        try:
            ...
            return render(request, 'purchase_premium.html', context)
